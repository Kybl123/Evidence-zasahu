...
function showPlaceGroup(group){
  ...
  $("side").innerHTML=
    `<div class="head">
      <div class="ico" style="background:${c[1]}">${c[2]}</div>
      <div>
        <h2>${esc(arr[0]?.type||"Zásah")}</h2>
        <div>${arr.length} zásah${arr.length===1?"":"ů"} na tomto místě</div>
      </div>
    </div>

    <div class="meta">
      <b>Souřadnice</b>
      <span>${primary.latitude.toFixed(5)}, ${primary.longitude.toFixed(5)}</span>
    </div>
    ...
    ${
      arr.map(i=>
        `<div class="record">
          <div class="recordtop">
            <strong>${esc(i.incident_date)}</strong>
            <span>${esc(i.alarm_level)}</span>
          </div>

          <div><b>${esc(i.type)}</b></div>

          <div>${esc(i.description||"Bez popisu")}</div>
          ...
          <div class="actions">
            <button class="edit" onclick="editIncident('${i.id}')">
              ✏️ Upravit
            </button>

            <button class="danger" onclick="deleteIncident('${i.id}')">
              🗑️ Smazat
            </button>
          </div>
        </div>`
      ).join("")
      || "<p>Na tomto místě zatím není žádný zásah.</p>"
    }

    <div class="actions">
      <button class="primary" onclick="addTo('${primary.id}')">
        ＋ Přidat zásah sem
      </button>

      ${
        group.length===1
        ? `<button onclick="deletePlace('${primary.id}')">Smazat místo</button>`
        : ""
      }
    </div>`;
}
